<x-layouts.guest title="Create account">
    ...
    <form method="POST" action="{{ route('register') }}" class="space-y-4" x-data="{ show: false }">
        @csrf
        <x-ui.input label="Full name" name="name" icon="user" :value="old('name')" placeholder="Aisha Rahman" autofocus required />
        <x-ui.input label="Email address" name="email" type="email" icon="mail" :value="old('email')" placeholder="you@example.com" required />
        ...
        <div class="space-y-1.5">
            ...
            <div class="relative">
                ...
                <input id="password" name="password" :type="show ? 'text' : 'password'" placeholder="At least 8 characters" required
        ...
        <x-ui.input label="Confirm password" name="password_confirmation" type="password" icon="lock" placeholder="Repeat your password" required />

        <label class="flex cursor-pointer items-start gap-2 text-sm">
            <input type="checkbox" name="terms" required class="mt-0.5 size-4 rounded border-input text-primary focus:ring-primary">
        ...
        <x-ui.button type="submit" class="w-full" size="lg">Create account <i data-lucide="arrow-right" class="rtl-flip"></i></x-ui.button>
    </form>
