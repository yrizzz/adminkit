@php
    ...
    /* Server-side FOUC: bake active-nav colour from cookie so it's correct
       on both hard reload AND wire:navigate (no JS required). */
    $_accentMap = [
    ...
    $_isColored = ($_sb === 'primary'
        || str_starts_with($_sb, 'gradient')
        || str_starts_with($_sb, 'image')
        || $_sb === 'custom_gradient');
    $_isLight = ($_sb === 'light');

    if ($_isColored) {
        $_bg = 'rgba(255,255,255,.95)'; $_fg = 'hsl(' . $_p . ')';
    } elseif ($_isLight) {
        $_bg = 'hsl(' . $_p . ')';     $_fg = '#fff';
    } else {
        $_bg = 'hsl(' . $_p . '/.15)'; $_fg = 'hsl(' . $_p . ')';
    }
    $_bar     = 'hsl(' . $_p . ')';
    $_foucCss = '.nav-link.active:not(.nav-sub){background:' . $_bg . '!important;color:' . $_fg . '!important}'
              . '.nav-link.active:not(.nav-sub)::before{background:' . $_bar . '!important}';
@endphp
<meta charset="utf-8">
...
{{-- Active-nav FOUC fix: server-rendered so wire:navigate picks up correct colours immediately --}}
...
<script>
    (function () {
        try {
            ...
            /* JS fallback: update #ak-fouc in case localStorage differs from cookie */
            var am = {blue:'221 83% 53%',violet:'262 83% 58%',green:'142 71% 45%',rose:'347 77% 50%',orange:'25 95% 53%',amber:'38 92% 50%',teal:'173 80% 40%'};
            var ac = get('ak_accent', 'blue'), p = am[ac] || '221 83% 53%';
            var sb = get('ak_sb_color', 'dark');
            var col = (sb === 'primary' || sb.indexOf('gradient') === 0 || sb.indexOf('image') === 0 || sb === 'custom_gradient');
            var lit = (sb === 'light');
            var css = col
            ...
            /* Suppress transitions on first paint to avoid animated colour jumps */
