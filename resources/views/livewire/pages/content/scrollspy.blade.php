<div>
    ...
    <div x-data="{ active: 'introduction' }" class="grid grid-cols-1 gap-4 lg:grid-cols-[220px_1fr]">
        <aside class="lg:sticky lg:self-start" :class="$store.ui.navbarFixed ? 'lg:top-20' : 'lg:top-4'">
        ...
        <div class="space-y-4">
            @foreach ($sections as $s)
                <section id="sec-{{ $s['id'] }}" :style="{ scrollMarginTop: $store.ui.navbarFixed ? '96px' : '24px' }" x-intersect.margin.0px.0px.-72%.0px="active = '{{ $s['id'] }}'">
    ...
    <div x-data="{ active: 'overview' }">
        ...
        @endphp
        <div class="sticky z-10 mb-4 flex gap-1 overflow-x-auto rounded-xl border border-border bg-card/80 p-1.5 backdrop-blur no-scrollbar"
             :class="$store.ui.navbarFixed ? 'top-16' : 'top-0'">
            @foreach ($tabs as $t)
        ...
        <div class="space-y-4">
            @foreach ($tabs as $t)
                <section id="tab-{{ $t['id'] }}" :style="{ scrollMarginTop: $store.ui.navbarFixed ? '136px' : '72px' }" x-intersect.margin.0px.0px.-72%.0px="active = '{{ $t['id'] }}'">
